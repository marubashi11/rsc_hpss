...
''' 訓練予測の読み込み '''
O_feature = pickle.load(open(os.path.join(model_dir, "O~-train_feature.txt"), "rb"))
H_feature = pickle.load(open(os.path.join(model_dir, "H~-train_feature.txt"), "rb"))
P_feature = pickle.load(open(os.path.join(model_dir, "P-train_feature.txt"), "rb"))
''' 訓練予測のスタックデータを作成 '''
...
if classifier == "SVM":
    ...
    clf = SVC(kernel="linear")
''' 分類器の訓練 '''
clf.fit(feature_stack, y_train_)
''' 分類器の保存 '''
